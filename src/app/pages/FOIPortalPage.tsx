...
import { Search, Plus, Info, Check, Clock, FileText, ArrowLeft, X, ChevronDown } from 'lucide-react';
...
function FOIRequestModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const validateMobile = (number: string) => {
    if (!number) { setMobileError(''); return true; }
    if (!number.startsWith('9')) { setMobileError('Number must start with 9'); return false; }
    setMobileError('');
    return true;
  };
  ...
  const handleMobileBlur = () => { setMobileTouched(true); validateMobile(form.mobile); };
  const handleEmailBlur = () => { setEmailTouched(true); validateEmail(form.email); };
  ...
  const validateFile = (file: File): boolean => {
    if (file.type !== 'application/pdf') { setFileError('Only PDF files under 5MB are accepted.'); return false; }
    if (file.size > 5 * 1024 * 1024) { setFileError('Only PDF files under 5MB are accepted.'); return false; }
    setFileError('');
    return true;
  };
  ...
  const handleFileDrop = (e: React.DragEvent) => {
    ...
    if (file && validateFile(file)) setForm({ ...form, file });
  };
  ...
  const renderDropdown = (
    label: string, value: string, isOpenState: boolean,
    setOpen: (v: boolean) => void, options: string[],
    fieldName: string, required = true,
  ) => (
    <div style={{ marginBottom: '16px' }}>
      <label style={labelStyle}>{label} {required && <span style={{ color: '#AA0924' }}>*</span>}</label>
      <div style={{ border: isOpenState ? '1.5px solid #AA0924' : '1px solid #E5E7EB', borderRadius: '8px', backgroundColor: '#FFFFFF', overflow: 'hidden', transition: 'border-color 0.15s ease' }}>
        <button
          type="button"
          onClick={() => setOpen(!isOpenState)}
          className="flex items-center justify-between w-full text-left"
          style={{ width: '100%', height: '48px', padding: '0 16px', fontSize: '14px', fontFamily: F, cursor: 'pointer', color: value ? '#000000' : '#9CA3AF', border: 'none', background: 'transparent' }}
        >
          <span>{value || options[0]}</span>
          <ChevronDown className="w-4 h-4 transition-transform" style={{ color: '#9CA3AF', transform: isOpenState ? 'rotate(180deg)' : 'rotate(0deg)' }} />
        </button>
        {isOpenState && (
          <div>
            {options.slice(1).map((opt) => (
              <button
                ...
                style={{ padding: '14px 16px', fontSize: '14px', fontFamily: F, color: value === opt ? '#AA0924' : '#1A1A1A', fontWeight: value === opt ? 600 : 500, border: 'none', borderTop: '1px solid #E5E7EB', background: 'transparent', cursor: 'pointer', display: 'block', width: '100%' }}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!submitted && (
          <div className="shrink-0 p-6 md:p-8 border-b border-[#E0E0E0]">
            <div className="flex items-start justify-between">
              <div>
                <h2 style={{ fontWeight: 700, fontSize: '24px', color: '#000000', fontFamily: F, lineHeight: '1.2', marginBottom: '10px' }}>
                  FOI Request Form
                </h2>
              ...
              <button
                ...
                style={{ width: '36px', height: '36px', borderRadius: '50%', border: 'none', background: 'transparent', cursor: 'pointer', marginTop: '-4px' }}
              >
        ...
        {submitted ? (
          /* ── Success State ── */
          <div className="flex-1 flex flex-col items-center justify-center" style={{ padding: '48px 32px', textAlign: 'center' }}>
            <div style={{ width: '60px', height: '4px', backgroundColor: '#AA0924', marginBottom: '32px' }} />
            <div className="flex items-center justify-center" style={{ width: '80px', height: '80px', borderRadius: '50%', backgroundColor: '#ECFDF5', marginBottom: '24px' }}>
              <Check className="w-10 h-10" style={{ color: '#28A745' }} strokeWidth={3} />
            </div>
            <h3 style={{ fontWeight: 700, fontSize: '22px', color: '#000000', fontFamily: F, marginBottom: '12px', lineHeight: '1.3' }}>
              Request Submitted Successfully!
            </h3>
            <p style={{ fontSize: '14px', fontFamily: F, fontWeight: 400, color: '#6B7280', lineHeight: '1.7', maxWidth: '400px', marginBottom: '40px', textAlign: 'center' }}>
              Your request has been sent to the ICSSC. Please check your email for the reference number and further updates.
            </p>
            <button
              onClick={handleClose}
              className="hover:opacity-90 transition-opacity"
              style={{ height: '48px', padding: '0 40px', backgroundColor: '#AA0924', borderRadius: '8px', border: 'none', color: '#FFFFFF', fontWeight: 700, fontSize: '16px', fontFamily: F, cursor: 'pointer' }}
            >
              CLOSE
            </button>
          </div>
              ...
              <form id="foi-form" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <input type="text" name="firstName" value={form.firstName} onChange={handleChange} placeholder="First Name" required style={inputStyle} onFocus={(e) => (e.target.style.borderColor = '#AA0924')} onBlur={(e) => (e.target.style.borderColor = '#E5E7EB')} />
                  </div>
                  <div>
                    <label style={labelStyle}>Last Name <span style={{ color: '#AA0924' }}>*</span></label>
                    <input type="text" name="lastName" value={form.lastName} onChange={handleChange} placeholder="Last Name" required style={inputStyle} onFocus={(e) => (e.target.style.borderColor = '#AA0924')} onBlur={(e) => (e.target.style.borderColor = '#E5E7EB')} />
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="Email Address" required style={{ ...inputStyle, borderColor: hasEmailError ? '#AA0924' : '#E5E7EB' }} onFocus={(e) => (e.target.style.borderColor = '#AA0924')} onBlur={(e) => { e.target.style.borderColor = '#E5E7EB'; handleEmailBlur(); }} />
                  {hasEmailError && <p style={{ color: '#AA0924', fontSize: '12px', fontFamily: F, fontWeight: 400, marginTop: '6px' }}>{emailError}</p>}
                </div>

                <div style={{ marginBottom: '16px' }}>
                  <label style={labelStyle}>Mobile Number <span style={{ color: '#AA0924' }}>*</span></label>
                  <div className="flex items-center" style={{ width: '100%', height: '48px', border: `1px solid ${hasMobileError ? '#AA0924' : '#E5E7EB'}`, borderRadius: '8px', backgroundColor: '#FFFFFF', transition: 'border-color 0.15s ease' }}>
                    <span style={{ padding: '0 14px 0 16px', fontFamily: F, fontWeight: 400, fontSize: '14px', color: '#6B7280', userSelect: 'none', flexShrink: 0, lineHeight: '48px' }}>+63</span>
                    <div style={{ width: '1px', height: '26px', backgroundColor: '#D1D5DB', flexShrink: 0 }} />
                    <input type="tel" name="mobile" value={form.mobile} onChange={handleMobileChange} placeholder="9XX XXX XXXX" required style={{ flex: 1, height: '100%', padding: '0 16px 0 14px', border: 'none', outline: 'none', fontSize: '14px', fontFamily: F, color: '#000000', backgroundColor: 'transparent', borderRadius: '0 8px 8px 0' }} onBlur={handleMobileBlur} />
                  </div>
                  {hasMobileError && <p style={{ color: '#AA0924', fontSize: '12px', fontFamily: F, fontWeight: 400, marginTop: '6px' }}>{mobileError}</p>}
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div className="w-full transition-colors flex" style={{ border: '1px solid #E5E7EB', borderRadius: '8px', backgroundColor: '#FFFFFF', padding: '4px' }}>
                    <textarea name="document" value={form.document} onChange={handleChange} placeholder="Describe the specific document or information you are requesting..." required rows={3} className="w-full flex-1 outline-none focus:outline-none" style={{ fontSize: '14px', fontFamily: F, color: '#000000', backgroundColor: 'transparent', border: 'none', height: 'auto', padding: '10px 12px', resize: 'vertical', minHeight: '80px' }} />
                  </div>
                </div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <div className="w-full transition-colors flex" style={{ border: '1px solid #E5E7EB', borderRadius: '8px', backgroundColor: '#FFFFFF', padding: '4px' }}>
                    <textarea name="reason" value={form.reason} onChange={handleChange} placeholder="Please describe the purpose and reason for your request..." required rows={4} className="w-full flex-1 outline-none focus:outline-none" style={{ fontSize: '14px', fontFamily: F, color: '#000000', backgroundColor: 'transparent', border: 'none', height: 'auto', padding: '10px 12px', resize: 'vertical', minHeight: '102px' }} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div
                    ...
                    style={{ border: '2px dashed #D1D5DB', borderRadius: '8px', padding: '24px', textAlign: 'center', cursor: 'pointer', backgroundColor: '#FAFAFA', transition: 'border-color 0.15s ease' }}
                    onClick={() => document.getElementById('foi-file-input')?.click()}
                    onMouseEnter={(e) => (e.currentTarget.style.borderColor = '#AA0924')}
                    onMouseLeave={(e) => (e.currentTarget.style.borderColor = '#D1D5DB')}
                  >
                    <input id="foi-file-input" type="file" accept=".pdf" onChange={handleFileSelect} style={{ display: 'none' }} />
                    {form.file ? (
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button type="button" onClick={(e) => { e.stopPropagation(); setForm({ ...form, file: null }); }} className="hover:bg-[#F3F4F6] transition-colors" style={{ background: 'none', border: 'none', cursor: 'pointer', marginLeft: '4px', borderRadius: '50%', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      ...
                      <>
                        ...
                        <p style={{ fontSize: '12px', fontFamily: F, color: '#9CA3AF' }}>PDF only (Max 5MB)</p>
                      </>
                    )}
                  </div>
                  {fileError && <p style={{ color: '#AA0924', fontSize: '12px', fontFamily: F, fontWeight: 400, marginTop: '6px' }}>{fileError}</p>}
                </div>
              </form>
            </div>

            {/* ── Fixed Footer ── */}
            <div className="shrink-0 px-6 md:px-8 border-t border-[#E0E0E0] flex flex-col justify-center" style={{ minHeight: '80px', paddingTop: '12px', paddingBottom: '12px' }}>
              {submitError && (
                <p style={{ color: '#AA0924', fontSize: '13px', fontFamily: F, marginBottom: '8px' }}>
                  {submitError}
                </p>
              )}
              <button
                ...
                style={{ height: '48px', padding: '0 36px', backgroundColor: '#AA0924', borderRadius: '8px', border: 'none', color: '#FFFFFF', fontWeight: 700, fontSize: '16px', fontFamily: F, cursor: isSubmitting ? 'not-allowed' : 'pointer', opacity: isSubmitting ? 0.6 : 1 }}
              >
      ...
      {viewMode === 'list' ? (
        <>
            ...
            <div className="max-w-[1280px] mx-auto px-6 md:px-20">
              <div className="flex justify-center mb-8">
                <div className="relative w-full max-w-lg">
                  ...
                  <input type="text" placeholder="Enter Search Keyword" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-12 pr-5 py-3.5 border border-[#E0E0E0] rounded-xl focus:outline-none focus:border-[#AA0924] transition-colors shadow-sm" style={{ fontFamily: F, fontSize: '15px' }} />
                </div>
              </div>
              ...
              <div className="border-b-2 border-[#E5E7EB] overflow-x-auto no-scrollbar -mx-6 px-6 md:mx-0 md:px-0">
                <div className="flex min-w-max md:w-full">
                  {tabs.map((tab) => {
                    ...
                    return (
                      <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`shrink-0 transition-all border-b-4 ${isActive ? 'border-[#AA0924] text-[#AA0924] font-bold' : 'border-transparent text-[#666]'} py-4 px-6 md:flex-1 text-center whitespace-nowrap text-[13px] tracking-wide uppercase`} style={{ fontFamily: F }}>
                        {tab.label}
                      </button>
                    );
                  })}
              ...
              <div className="flex justify-center md:justify-end py-8">
                <button onClick={() => setModalOpen(true)} className="w-full md:w-auto flex items-center justify-center gap-2 shrink-0 bg-[#AA0924] hover:bg-[#880718] text-white px-8 h-12 rounded-full transition-all font-bold text-sm shadow-md active:scale-95">
          ...
          <section className="bg-white pt-0 pb-20 md:pb-32">
            <div className="max-w-[1280px] mx-auto px-6 md:px-20">
              {loading && <div className="text-center py-16 text-[#555555]">Loading requests…</div>}
              {error && <div className="text-center py-16 text-[#AA0924]">{error}</div>}
              {!loading && !error && filteredRequests.length === 0 ? (
                <div className="text-center py-16">
                  ...
                  <p style={{ fontSize: '14px', color: '#9CA3AF', marginTop: '4px' }}>Try adjusting your search or filter criteria.</p>
                </div>
              ) : (
                  ...
                  {filteredRequests.map((req) => {
                    ...
                    return (
                      <div key={req.id} className="relative bg-white overflow-hidden border border-[#E0E0E0] hover:border-[#AA0924] transition-all shadow-sm hover:shadow-md" style={{ borderRadius: '8px' }}>
                        <div className="absolute top-0 left-0 bottom-0" style={{ width: '4px', backgroundColor: '#AA0924', borderTopLeftRadius: '8px', borderBottomLeftRadius: '8px' }} />
                        <div className="p-6 md:p-8">
                          <div className="flex justify-start md:block mb-4 md:mb-0">
                            <span className="md:absolute md:top-6 md:right-6 text-[10px] md:text-[11px] font-bold px-3 py-1 md:px-4 md:py-1.5 rounded-full uppercase tracking-wider" style={{ color: s.text, backgroundColor: s.bg, fontFamily: F }}>{s.label}</span>
                          </div>
                          <h3 className="text-lg md:text-xl font-bold text-black leading-snug mb-4 md:pr-40" style={{ fontFamily: F }}>{req.title}</h3>
                          <p style={{ fontSize: '14px', fontFamily: F, fontWeight: 400, color: '#666666', margin: 0, lineHeight: '1.5', marginBottom: '4px' }}>Reference No.: <span style={{ color: '#AA0924' }}>{req.refId}</span></p>
                          <p style={{ fontSize: '14px', fontFamily: F, fontWeight: 400, color: '#666666', margin: 0, lineHeight: '1.5', marginBottom: '4px' }}>{dateLabel}: <span style={{ color: '#AA0924' }}>{req.publishedDate}</span></p>
                          ...
                          <div className="flex justify-end mt-6">
                            <button onClick={() => handleViewStatus(req)} className="w-full md:w-auto h-11 px-6 border-2 border-[#AA0924] text-[#AA0924] bg-white hover:bg-[#AA0924] hover:text-white transition-all font-bold text-sm tracking-wide rounded-lg uppercase" style={{ fontFamily: F }}>
                              VIEW STATUS
                            </button>
                          </div>
                        </div>
        ...
        <>
          <section className="bg-white pt-20 md:pt-32 pb-20 md:pb-32">
            <div className="max-w-[1280px] mx-auto px-6 md:px-20">
              <button onClick={handleBackToList} className="flex items-center gap-2 hover:text-[#AA0924] transition-colors" style={{ background: 'none', border: 'none', cursor: 'pointer', fontWeight: 400, fontSize: '14px', color: '#666666', fontFamily: F, marginBottom: '32px' }}>
              ...
              <div style={{ marginBottom: '24px' }}>
                <p style={{ fontSize: '14px', fontFamily: F, fontWeight: 400, color: '#666666', margin: 0, lineHeight: '1.7' }}>Reference No.: <span style={{ color: '#AA0924' }}>{selectedRequest.refId}</span></p>
                <p style={{ fontSize: '14px', fontFamily: F, fontWeight: 400, color: '#666666', margin: 0, lineHeight: '1.7' }}>{selectedRequest.status === 'pending' ? 'Submitted on' : 'Published on'}: <span style={{ color: '#AA0924' }}>{selectedRequest.publishedDate}</span></p>
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-center gap-3" style={{ backgroundColor: sc.bg, border: `1px solid ${borderColor}`, borderRadius: '8px', padding: '16px 20px', marginBottom: '40px' }}>
                    ...
                    <p style={{ fontWeight: 400, fontSize: '14px', color: sc.text, fontFamily: F, margin: 0 }}>{messages[selectedRequest.status]}</p>
                  </div>
                );
              })()}
              ...
              <div className="bg-white border border-[#E0E0E0] hover:border-[#AA0924] transition-all shadow-sm hover:shadow-md" style={{ borderRadius: '8px', padding: '32px' }}>
